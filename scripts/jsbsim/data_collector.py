...
from algorithms.ppo_replaybuffer import ReplayBuffer


class BaselineDataCollector(object):
    ...
    def _make_action(self, ego_cur_act_list: list):
        ...
        acts = []
        for i in range(self.num_envs):
            ego_cur_act = ego_cur_act_list[i]
            enm_cur_act = OrderedDict({"aileron": 20, "elevator": 18.6, "rudder": 20,"throttle": 0})
            if self.red_flag[i]:
                act = {'red_fighter': ego_cur_act, 'blue_fighter': enm_cur_act}
            else:
                act = {'red_fighter': enm_cur_act, 'blue_fighter': enm_cur_act}
            acts.append(act)
    ...
    def collect_data(self, ego_net_params, enm_net_params, hyper_params=None, agent_id=None):
        ...
        while not flag_rollout_abort:
            ...
            ego_next_obss, enm_next_obss, ego_rewards, enm_rewards, dones, env_infos = self.step(ego_cur_acts)
            rollout_step += 1
    ...
    def evaluate(self, ego_net_params, enm_net_params, eval_num=1):
        print('#####################################################################################')
        print('\nStart evaluating...')
    ...
    def render(self, ego_net_params, enm_net_params):
