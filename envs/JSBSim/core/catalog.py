...
class ExtraCatalog(Property, Enum):
    ...
    velocities_u_mps = Property("velocities/u-mps", "body frame x-axis velocity [m/s]", -700, 700, \
        access="R", update=lambda sim: sim.set_property_value(ExtraCatalog.velocities_u_mps, \
            sim.get_property_value(JsbsimCatalog.velocities_u_fps) * 0.3048))

    velocities_v_mps = Property("velocities/v-mps", "body frame y-axis velocity [m/s]", -700, 700, \
        access="R", update=lambda sim: sim.set_property_value(ExtraCatalog.velocities_v_mps, \
            sim.get_property_value(JsbsimCatalog.velocities_v_fps) * 0.3048))

    velocities_w_mps = Property("velocities/w-mps", "body frame z-axis velocity [m/s]", -700, 700, \
        access="R", update=lambda sim: sim.set_property_value(ExtraCatalog.velocities_w_mps, \
