...
from .base_runner import Runner, SharedReplayBuffer
...
class MPERunner(Runner):
    ...
    def load(self):
        ...
        self.share_obs_space = spaces.Box(low=-np.inf, high=np.inf, shape=(self.obs_space.shape[0]*self.num_agents, ))
        ...
        # buffer
        if self.num_agents > 2 and self.use_selfplay:
        ...
        for episode in range(episodes):
            ...
            for step in range(self.buffer_size):
                ...
                if self.env_name == "MPE":
                    actions_env = np.squeeze(np.eye(self.envs.action_space[0].n)[actions], axis=2)
                else:
                    actions_env = actions
    ...
    def warmup(self):
        ...
        # TODO: How to deal with selfplay and shared obs
